...
static inline 
__m512 diffusion_formula_vectorized_16(__m512 left, __m512 center, __m512 right, __m512 up, __m512 down, __m512 vec_4, __m512 vec_diff_const) {
	// calculate result = center + diff_const * ((left+right+up+down)/4 - center)
	__m512 result_vec, tmp_vec;

	// add left and right (8 floats in each vector, start one row down)
	tmp_vec = _mm512_add_ps(left, right);

	// add up and down (8 floats in each vector)
	result_vec = _mm512_add_ps(up, down);

	// add the results (8 floats in each vector)
	result_vec = _mm512_add_ps(tmp_vec, result_vec);

	// divide by 4
	result_vec = _mm512_div_ps(result_vec, vec_4);

	// subtract center from result 
	result_vec = _mm512_sub_ps(result_vec, center);

	// multiply by diffusion constant
	result_vec = _mm512_mul_ps(result_vec, vec_diff_const);

	// add to center
	result_vec = _mm512_add_ps(result_vec, center);
...
void perform_diffusion_vectorized_cols(int rows, int full_width, int width, float *matrix_prev, float *matrix_next) {
	...
	__m512 vec_4 = _mm512_set1_ps(4.f);
	__m512 vec_diff_const = _mm512_set1_ps(diff_const);

	// perform the diffusion on n_vec cols at the same time
	// remember the padding of 1 in each direction
	for (int i_row=1; i_row<(rows+1); i_row++) {
		int i_col = 1;
		for (; i_col<(width+1)-n_vec; i_col+=n_vec) {
			__m512 left   = _mm512_loadu_ps(&matrix_prev[ i_row   *full_width + i_col-1]);
			__m512 center = _mm512_loadu_ps(&matrix_prev[ i_row   *full_width + i_col  ]);
			__m512 right  = _mm512_loadu_ps(&matrix_prev[ i_row   *full_width + i_col+1]);
			__m512 up 	  = _mm512_loadu_ps(&matrix_prev[(i_row-1)*full_width + i_col  ]);
			__m512 down   = _mm512_loadu_ps(&matrix_prev[(i_row+1)*full_width + i_col  ]);

			__m512 result_vec = diffusion_formula_vectorized_16(left, center, right, up, down, vec_4, vec_diff_const);
			_mm512_storeu_ps(&matrix_next[i_row*full_width+i_col], result_vec);
		}

		// calculate the remaining columns normally
		for (; i_col<(width+1); i_col++) {
			float left   = matrix_prev[ i_row   *full_width + i_col-1];
			float center = matrix_prev[ i_row   *full_width + i_col  ];
			float right  = matrix_prev[ i_row   *full_width + i_col+1];
			float up 	 = matrix_prev[(i_row-1)*full_width + i_col  ];
			float down   = matrix_prev[(i_row+1)*full_width + i_col  ];

			float value = left + right + up + down;
			matrix_next[i_row*full_width+i_col] = center + diff_const * (value/4.f - center);
		}
	}
}
